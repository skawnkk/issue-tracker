...
import SelectedTabUser from 'components/createIssuePage/issueDetailOption/SelectedTabUser';
import SelectedTabLabel from 'components/createIssuePage/issueDetailOption/SelectedTabLabel';
import SelectedTabMilestone from 'components/createIssuePage/issueDetailOption/SelectedTabMilestone';
...
function IssueDetailOption({ id }: Props): ReactElement {
  ...
  const checkedList: { [key: string]: ReactElement | Array<ReactElement> | null } = {
    assignee: selectTabToRender.assignee?.map((user) => <SelectedTabUser key={user.id} user={user} />),
    label: selectTabToRender.label?.map((label) => <SelectedTabLabel key={label.id} label={label} />),
    milestone: selectTabToRender.milestone ? (
      <SelectedTabMilestone key={selectTabToRender.milestone.id} milestone={selectTabToRender.milestone} />
    ) : null,
  };
